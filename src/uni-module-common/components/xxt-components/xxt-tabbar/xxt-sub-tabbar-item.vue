    ...
    <template v-else>
      <view class="u-tabbar-item__icon">
        ...
        <TuiBadge
          ...
          absolute
          top="0"
          :right="dot ? '34rpx' : badge > 9 ? '14rpx' : '20rpx'"
          :dot="dot"
          type="danger"
          :scale-ratio="1"
          >{{ badge || '' }}</TuiBadge
 ...
 *
 */
import TuiBadge from '../../thorui/tui-badge/tui-badge.vue';
import { $parent, addStyle } from '@/uni-module-common/helper';
...
export default {
  ...
  watch: {
    name(newvalue, oldvalue) {
      console.log('watch000-----', newvalue, oldvalue);
    }
  },
  created() {
    this.init();
  ...
  methods: {
    getParentData(parentName = '') {
      ...
      if (this.parent.children) {
        ...
        !this.parent.children.includes(this) && this.parent.children.push(this);
      }
    ...
    init() {
      ...
      console.log('updateFromParent-----name----', this.name.split('?')[0]);
      console.log('updateFromParent-----index----', index);
      console.log('updateFromParent-----isActive----', this.isActive);
      console.log('updateFromParent-----parentData----', this.parentData.value);
    },
    updateParentData() {
      // 此方法在mixin中
    ...
    // 此方法将会被父组件u-tabbar调用
    updateFromParent() {
      console.log('updateFromParent-----', this.name.split('?')[0]);
      // 重新初始化
      this.init();
    },
    clickHandler() {
