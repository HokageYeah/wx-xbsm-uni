<template>
  <scroll-view
    ...
    :class="[
      isFixed && !isSticky ? 'tui-tabs__fixed' : '',
      isSticky ? 'tui-tabs__sticky' : '',
      classView
    ]"
    :style="{
      height: `${height}rpx`,
  ...
  >
    <view class="tui-tabs__wrap" @touchstart="touchStart" @touchend="touchEnd">
      <view
      ...
      >
        <view
          ...
          :class="currentTab == index ? 'tui-tabs__item__active' : ''"
          :style="{ height: `${height}rpx` }"
          :data-index="index"
          @tap="handleClick"
        >
          <view
            ...
            :class="[childClass, currentTab == index ? 'tui-item__child__active' : '']"
            :style="{
          ...
          >
            <slot name="title" :tab="item">{{ item?.[field] || item }}</slot>
          </view>
        </view>
        <view
...
export default {
  ...
  data() {
    const childClass = `tui_10_${Math.ceil(Math.random() * 10e5).toString(36)}`;
    const classView = `tui_01_${Math.ceil(Math.random() * 10e5).toString(36)}`;
    return {
      childClass,
      classView,
      /* 未渲染数据 */
  ...
  methods: {
    ...
    scrollByIndex(tabCur, needTransition = true) {
      const item = this.tabItems[tabCur];
      if (!item) return;
      ...
      if (this.scroll) {
        ...
        const scrollLeft = itemLeft - (this.windowWidth - itemWidth) / 2;
    ...
    touchEnd(event) {
      ...
      if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
        if (deltaX < 0) {
          // 往左
          const scrollLeft = this.scrollLeft + Math.abs(deltaX);
          const maxScrollLeft =
            this.tabItems[this.tabItems.length - 1].left -
            this.tabItems[this.tabItems.length - 1].width;
          if (this.touchScroll) {
            this.scrollLeft = scrollLeft < maxScrollLeft ? scrollLeft : maxScrollLeft;
          }
        } else if (deltaX > 0) {
          // 往右
          const scrollLeft = this.scrollLeft - Math.abs(deltaX);
