<template>
  <view class="tui-searchbar__box" :style="{ backgroundColor, padding }">
    <slot></slot>
    <view class="tui-search-bar__form" :style="{ height }">
      <view
        v-if="showInput"
        class="tui-search-bar__box"
        :class="{ 'tui-searchbar__blur': !isFocus && !searchState && showLabel && !disabled }"
        :style="{ height, borderRadius: radius, backgroundColor: inputBgColor }"
      >
        ...
        />
        <icon
          v-if="valueTxt.length > 0 && !disabled"
          type="clear"
          :size="clearSize"
    ...
    <view
      v-if="cancel && searchState && !valueTxt"
    ...
    >
    <view
      v-if="valueTxt && !disabled && searchState"
      class="tui-search-bar__cancel-btn"
      :style="{ color: getSearchColor }"
      @tap="search"
      >{{ searchText }}</view
    >
  </view>
