...
export default {
  ...
  data() {
    ...
    const cid = `id01_${Math.ceil(Math.random() * 10e5).toString(36)}`;
    const did = `id02_${Math.ceil(Math.random() * 10e5).toString(36)}`;
    // #endif
    return {
  ...
  methods: {
    ...
    // 默认(背景)圆环
    drawDefaultCircular() {
      const ctx = uni.createCanvasContext(this.defaultCanvasId, this);
      ...
      // 终止弧度
      const eAngle = Math.PI * (this.height ? 1 : 2) + this.sAngle;
    ...
    drawProgressCircular(startPercentage) {
      ...
      const time =
        this.percentage === 0 || this.duration < 50 ? 0 : this.duration / this.percentage;
      ...
      if (this.fontShow) {
        ...
        const fontColor =
          this.fontColor || (uni && uni.$tui && uni.$tui.color.primary) || '#5677fc';
      ...
      if (this.percentage === 0 || (this.counterclockwise && startPercentage === 100)) {
        ...
        const eAngle = ((2 * Math.PI) / 100) * startPercentage + this.sAngle;
        this.drawArc(ctx, eAngle);
      }
      setTimeout(() => {
