 ...
 */
export function getPx(value: string, unit = false) {
 ...
 */
export function guid(len = 32, firstU = true, radix: any) {
...
*/
export function $parent(this: any, name = undefined) {
 ...
 */
function timeFormat(dateTime: any, formatStr = 'yyyy-mm-dd') {
  let date;
  // 若传入时间为假值，则取当前时间
  if (!dateTime) {
    date = new Date();
 ...
 */
function timeFrom(timestamp: any, format = 'yyyy-mm-dd') {
 ...
 */
function toast(title: any, duration = 2000) {
  uni.showToast({
    title: String(title),
    icon: 'none',
    duration
 ...
 */
function priceFormat(number: any, decimals = 0, decimalPoint = '.', thousandsSeparator = ',') {
 ...
 */
function getDuration(value: any, unit = true) {
  const valueNum = parseInt(value);
  if (unit) {
    if (value.endsWith('s')) return value;
    return value > 30 ? `${value}ms` : `${value}s`;
  }
  if (value.endsWith('ms')) return valueNum;
  if (value.endsWith('s')) return valueNum > 30 ? valueNum : valueNum * 1000;
  return valueNum;
}
 ...
 */
function setProperty(obj: any, key: any, value: any) {
  if (!obj) {
    return;
  }
  // 递归赋值
  const inFn = function (_obj: any, keys: any, v: any) {
    // 最后一个属性key
    if (keys.length === 1) {
      _obj[keys[0]] = v;
      return;
...
function page() {
  ...
  // options转换成key=value的字符串，用于拼接
  let pageurl = `/${url}`;
  const optionAry = Object.entries(options);
  if (optionAry.length > 0) {
    pageurl += '?';
    optionAry.forEach(([key, value]) => {
      pageurl += `${key}=${value}&`;
    });
    pageurl = pageurl.slice(0, -1);
  }
  console.log('pages---pageurl---', pageurl);
  return pageurl;
}
